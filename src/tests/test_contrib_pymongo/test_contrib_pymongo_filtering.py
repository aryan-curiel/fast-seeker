...
from fast_seeker.contrib.pymongo.filtering import FilterModel, PyMongoFilterer
...
def test_odmantic_filterer_filter__should_assign_expected_find_expressions(
    filter_query, expected_expressions, pymongo_collection
):
    filterer = PyMongoFilterer()
    filterer.filter(pymongo_collection, filter_query)
    pymongo_collection.find.assert_called_once_with(expected_expressions)
